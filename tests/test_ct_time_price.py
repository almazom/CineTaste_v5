"""
Test ct-time-price tool contract validation and CLI behavior.
"""
...
def load_time_price_main():
    path = ROOT / "tools" / "ct-time-price" / "main.py"
    sys.path.insert(0, str(path.parent))
    spec = importlib.util.spec_from_file_location("ct_time_price_main", path)
...
class TestTimePriceCli:
    def test_ct_time_price_dry_run_emits_valid_contract(self, tmp_path: Path):
        output_file = tmp_path / "showtimes.json"

        result = subprocess.run(
            [
                sys.executable,
                str(ROOT / "tools" / "ct-time-price" / "main.py"),
                "--url",
        ...
        assert all("price" not in item for item in payload["showtimes"])

    def test_ct_time_price_without_date_uses_local_today(self):
        result = subprocess.run(
            [
                sys.executable,
                str(ROOT / "tools" / "ct-time-price" / "main.py"),
    ...
    def test_ct_time_price_fetch_failure_returns_unavailable_exit(self, monkeypatch, capsys):
        module = load_time_price_main()
